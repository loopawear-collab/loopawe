...
function itemTitle(it: CartItem) {
  // future-proof: productType indien aanwezig
...
export default function CheckoutPage() {
  ...
  async function onPlaceOrder() {
    // ✅ hard lock (anti double submit incl. enter spam)
    if (busy) return;
                  ...
                  <div className="mt-4 space-y-3">
                    ...
                    {items.length > 4 ? (
                      <p className="text-xs text-zinc-500">
                        + {items.length - 4} meer item(s) in je order
                      </p>
                    ) : null}
                  </div>
                </div>
